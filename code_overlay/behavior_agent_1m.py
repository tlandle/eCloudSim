...
class BehaviorAgent(object):
    ...
    def __init__(self, vehicle, carla_map, config_yaml, is_dist=False,
        ...
        # freeze-1l EPOCH_FENCE knob (read once; default ON). ON = the planner
        # skips a stale-owner (lower-epoch) forecast; OFF disables only the skip
        # while the [CONSUMEDEPOCH] fenced=1 flag and the _max_owner_epoch
        # bookkeeping still fire, so the lander sees the stale consumption.
        # Unset -> '1' -> fence on.
        import os as _oef
        self._epoch_fence = _oef.environ.get('EPOCH_FENCE', '1') != '0'
    ...
    def collision_manager(self, rx, ry, ryaw, waypoint, adjacent_check=False, is_left_turn_at_intersection=False, check_full_path=False):
        ...
        for pred in self.generated_predictions:
            ...
            # freeze-1l planner epoch fence (BEHAVIOR change; _epoch overlay
            # ONLY). Keep the highest owner epoch seen per actor and ignore any
            # forecast carrying a LOWER epoch, i.e. a stale-owner forecast the
            # planner would otherwise consume in the window where it briefly sees
            # both the old and the new owner. Source-side is unchanged (the old
            # owner keeps serving its bound vehicles = the measured
            # double-emission window); this only stops the destination planner
            # from acting on the superseded owner, making "a forecast from the
            # old owner is not consumed" true for a planner that ever sees both.
            # [CONSUMEDEPOCH] is still emitted either way, with fenced=1 when the
            # stale-owner pred is skipped and 0 when consumed, so the lander sees
            # the fence fire. Dormant on a single-planner flow run (one actor
            # never carries two owner epochs at one planner, and epochs are
            # monotone there); fires on a corridor route at the second crossing
            # (the planner has seen epoch 1 then epoch 2 for the shared actor).
            if not hasattr(self, '_max_owner_epoch'):
                self._max_owner_epoch = {}  # actor carla_id -> highest epoch seen
            _ce_actor = getattr(pred.obstacle_trajectory.obstacle, 'carla_id', -1)
            _ce_e = int(getattr(pred, 'owner_epoch', -1))
            _ce_fenced = 1 if (
                _ce_e >= 0 and _ce_actor >= 0
                and _ce_e < self._max_owner_epoch.get(_ce_actor, -1)) else 0
            try:
                logger.info(
                    "[CONSUMEDEPOCH] ego=%s actor=%s tick=%s consumed_epoch=%d "
                    "fenced=%d",
                    getattr(self.vehicle, 'id', '?'), _ce_actor,
                    getattr(self, '_tick_counter', -1), _ce_e, _ce_fenced)
            except Exception:  # noqa: BLE001
                pass
            if self._epoch_fence and _ce_fenced:
                # planner-side skip, gated on EPOCH_FENCE (kept + future-proofed
                # for a 2-stream path; dormant under delivery-by-binding, where
                # an upstream-bound planner never receives the newer epoch so
                # _ce_fenced stays 0). The source-gate in the edge is the active
                # fence. The [CONSUMEDEPOCH] fenced flag above still fires either
                # way so the lander sees would-have-fenced.
                continue  # do not consume the stale-owner (lower-epoch) forecast
            if _ce_e >= 0 and _ce_actor >= 0:
                self._max_owner_epoch[_ce_actor] = max(
                    self._max_owner_epoch.get(_ce_actor, -1), _ce_e)
    ...
    def _nearest_oncoming_ahead(self):
        ...
        # freeze-1m: closed-form KINEMATIC arrival from the delivered track's
        # (distance, current speed, carried acceleration). MTR full-walk + endpoint
        # are logged for comparison only, never the decision.
        arrival, src, a_used, a_src, meet_spd, full_walk, endpoint_extrap = \
            self._gate_arrival(best, best_speed, best_pred, ex, ey, cos_y, sin_y)
        cv_arr = (best / (7.0 + max(best_speed, 2.0))
                  if best < float('inf') else float('inf'))
        self._gate_arr_dbg = (
            round(cv_arr, 2) if cv_arr < float('inf') else None,        # 0 cv
            round(arrival, 2) if arrival < float('inf') else None,      # 1 kinematic (decision)
            round(a_used, 2),                                           # 2 accel used
            a_src,                                                      # 3 accel source
            round(meet_spd, 1) if meet_spd is not None else None,       # 4 meeting speed
            round(full_walk, 2) if full_walk is not None else None,     # 5 MTR full-walk (logged)
            round(endpoint_extrap, 2) if endpoint_extrap is not None else None,  # 6 endpoint (logged)
        )
        return best, best_speed, arrival

    def _gate_arrival(self, clear, speed, pred, ex, ey, cos_y, sin_y):
        """freeze-1m: closed-form KINEMATIC meeting time = first t solving
        clear - v*t - 0.5*a*t^2 <= OV*t (the ego advances into the opposing lane
        at OV=7 m/s), i.e. 0.5*a*t^2 + (v+OV)*t - clear = 0. a = the delivered
        track's carried acceleration (obs.gate_accel: migrated est_a while
        imported/immature, the destination's local estimate once mature, 0 for
        kf/handover snapshots and below the noise floor). a=0 reduces to
        clear/(v+OV) = the old CV gate (flow byte-identical). No speed cap (a
        scenario property the planner cannot know); the extrapolated meeting speed
        is returned for logging. The MTR full-forecast walk and the endpoint slope
        are computed for LOGGING only, never the decision. Returns
        (arrival, src, a_used, a_src, meeting_speed, full_walk, endpoint)."""
        OV = 7.0
        cv = (clear / (OV + max(speed, 2.0))
              if clear < float('inf') else float('inf'))
        if pred is None:
            return cv, 'no_oncoming', 0.0, 'none', None, None, None
        full_walk = self._full_walk_arrival(pred, ex, ey, cos_y, sin_y, OV)
        endpoint = self._endpoint_extrap_arrival(pred, ex, ey, cos_y, sin_y)
        obs = pred.obstacle_trajectory.obstacle
        a = float(getattr(obs, 'gate_accel', 0.0) or 0.0)
        a_src = getattr(obs, 'gate_accel_src', 'none')
        v = max(float(speed), 0.0)
        if clear >= float('inf'):
            return cv, 'no_oncoming', a, a_src, None, full_walk, endpoint
        B = v + OV
        if abs(a) < 1e-6:
            arr = clear / B if B > 1e-6 else float('inf')
        else:
            disc = B * B + 2.0 * a * clear  # roots of 0.5 a t^2 + B t - clear = 0
            if disc < 0:
                arr = float('inf')  # decelerates enough never to close: safe
            else:
                sq = disc ** 0.5
                cand = [r for r in ((-B + sq) / a, (-B - sq) / a) if r > 1e-6]
                arr = min(cand) if cand else float('inf')
        meeting_speed = (v + a * arr) if arr < float('inf') else None
        return arr, 'kin_' + a_src, a, a_src, meeting_speed, full_walk, endpoint

    def _full_walk_arrival(self, pred, ex, ey, cos_y, sin_y, OV=7.0):
        """LOGGED ONLY (never the decision, freeze-1m): meeting time from walking
        the MTR full 5s forecast predicted_trajectory_full to the ego's advancing
        front. Kept as a comparison column against the kinematic decision."""
        full = getattr(pred, 'predicted_trajectory_full', None)
        full_dt = float(getattr(pred, 'full_dt', 0.0) or 0.0)
        if not full or full_dt <= 0.0 or len(full) < 2:
            return None
        prev_ahead = None
        prev_t = 0.0
        for k, tf in enumerate(full):
            ahead = (tf.location.x - ex) * cos_y + (tf.location.y - ey) * sin_y
            t = (k + 1) * full_dt
            if ahead <= OV * t:
                if prev_ahead is not None:
                    denom = OV * (t - prev_t) - (ahead - prev_ahead)
                    f = ((prev_ahead - OV * prev_t) / denom
                         if abs(denom) > 1e-9 else 0.0)
                    return prev_t + (t - prev_t) * min(max(f, 0.0), 1.0)
                return t
            prev_ahead = ahead
            prev_t = t
        return None

    def _endpoint_extrap_arrival(self, pred, ex, ey, cos_y, sin_y):
        """LOGGED ONLY (never the decision): extrapolate the closing rate from the
        last two samples of the 1.25s backstop trajectory to the meeting. Kept as
        a comparison column against the full-forecast walk."""
        traj = getattr(pred, 'predicted_trajectory', None)
        if not traj or len(traj) < 2:
            return None
        dt = 0.05
        a_last = ((traj[-1].location.x - ex) * cos_y
                  + (traj[-1].location.y - ey) * sin_y)
        a_prev = ((traj[-2].location.x - ex) * cos_y
                  + (traj[-2].location.y - ey) * sin_y)
        v_close = (a_prev - a_last) / dt
        if v_close <= 1e-6:
            return None
        t_last = len(traj) * dt
        return (a_last + v_close * t_last) / (7.0 + v_close)
    ...
    def run_step(
        ...
        if self.do_overtake:
            _rc_clear, _rc_onc, _rc_arr = self._nearest_oncoming_ahead()
            _rc_need = 4.0 * (7.0 + max(_rc_onc, 2.0))  # CV need, logged
            _rc_tman = 4.0
            _following_gap = 7.0
                ...
                if self._ego_speed >= obstacle_speed - 5:
                    ...
                    if self.overtake_wait_counter > 0 and not self.do_overtake:
                        ...
                        if self.num_overtake_collisions > 1 or car_following_flag:
                            ...
                            _clear, _onc, _arr = self._nearest_oncoming_ahead()
                            ...
                            _go_dbg = getattr(self, '_gate_arr_dbg',
                                              (None,) * 7)
                            _cid = (self._nearest_onc_dbg[4]
                                    if getattr(self, '_nearest_onc_dbg', None)
                                    else -1)
                            logger.info("[GATE ARR] cid=%s cv=%s kin=%s accel=%s "
                                        "accel_src=%s meet_spd=%s full_walk=%s "
                                        "endpoint=%s t_man=%.1f clear=%.0f -> %s",
                                        _cid, _go_dbg[0], _go_dbg[1], _go_dbg[2],
                                        _go_dbg[3], _go_dbg[4], _go_dbg[5],
                                        _go_dbg[6], _t_man, _clear,
                                        'GO' if _arr >= _t_man else 'WAIT')
                            if os.environ.get('BEHAVIOR_DEBUG'):
                                print(f"[OT SIGHT] oncoming_ahead={_clear:.0f}m "
                                      f"onc_spd={_onc:.1f} need={_need:.0f}m -> "
                                      f"{'GO' if _arr >= _t_man else 'WAIT'} "
                            ...
                            if _arr < _t_man or self._ot_go_streak < 2:
