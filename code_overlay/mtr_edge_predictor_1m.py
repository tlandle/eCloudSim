...
class MTREdgePredictor:
    ...
    def _run_mtr(self, tracks: Dict[int, ObstacleTrajectory],
        ...
        for i, (tid, ot) in enumerate(zip(track_ids, otrajs)):
            ...
            def _to_world_full(local_traj):
                # freeze-1m: the model's FULL horizon (all model steps at the
                # native self._dt, ~0.2s, = 5s) transformed to world, WITHOUT the
                # 1.25s resample _to_world applies. Read ONLY by the overtake gate
                # (arrival at the pass zone). The backstop keeps reading the 1.25s
                # predicted_trajectory (pred_tfs) unchanged; num_output_steps
                # unchanged. This is the 5s forecast the paper says the planner
                # consumes (FDE defined at 3s/5s), not a slope proxy.
                out = []
                for j in range(len(local_traj)):
                    lx = float(local_traj[j, 0])
                    ly = float(local_traj[j, 1])
                    wx = cx + lx * cos_h - ly * sin_h
                    wy = cy + lx * sin_h + ly * cos_h
                    out.append(Transform(
                        location=Location(x=wx, y=wy, z=float(cur_z)),
            ...
            # freeze-1m: full 5s horizon for the overtake gate only (gate-local).
            pred.predicted_trajectory_full = _to_world_full(best)
            pred.full_dt = float(self._dt)
            predictions.append(pred)
    ...
    def _cv_from_migrated(self, ot: ObstacleTrajectory, mig_vel,
        ...
        dt = 0.05  # 20Hz tick
        # freeze-1l accel path: constant-acceleration extrapolation when the
        # destination estimated |a| above the noise floor for this imported
        # track (carried on the obstacle as mig_accel_mps2; None/0 otherwise).
        # a == 0 reduces EXACTLY to the CV forecast below (byte-identical).
        _a = getattr(ot.obstacle, 'mig_accel_mps2', None) or 0.0
        v0 = (vx * vx + vy * vy) ** 0.5
        ux, uy = (vx / v0, vy / v0) if v0 > 1e-6 else (0.0, 0.0)
        pred_tfs = []
        for step in range(self.num_output_steps):
            t = (step + 1) * dt
            if _a:
                t_eff = min(t, v0 / (-_a)) if (_a < 0.0 and v0 > 0.0) else t
                disp = v0 * t_eff + 0.5 * _a * t_eff * t_eff
                px, py = cur.location.x + ux * disp, cur.location.y + uy * disp
            else:
                px, py = cur.location.x + vx * t, cur.location.y + vy * t
            pred_tfs.append(Transform(
                location=Location(x=px, y=py, z=cur.location.z),
        ...
        pred = ObstaclePrediction(
            ...
            predicted_trajectory=pred_tfs,
            source_tick=source_tick, publish_tick=publish_tick)
        # freeze-1m: full 5s horizon for the overtake gate, same constant-accel
        # model extended past the 1.25s backstop window (this immature-import path
        # carries the migrated acceleration mig_accel_mps2 = est_a that tracks GT).
        _n_full = int(round(5.0 / dt))
        full_tfs = []
        for step in range(_n_full):
            t = (step + 1) * dt
            if _a:
                t_eff = min(t, v0 / (-_a)) if (_a < 0.0 and v0 > 0.0) else t
                disp = v0 * t_eff + 0.5 * _a * t_eff * t_eff
                ...
                px, py = cur.location.x + vx * t, cur.location.y + vy * t
            full_tfs.append(Transform(
                location=Location(x=px, y=py, z=cur.location.z),
                rotation=Rotation(yaw=cur.rotation.yaw)))
        pred.predicted_trajectory_full = full_tfs
        pred.full_dt = dt
        return pred
