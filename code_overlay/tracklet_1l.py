    ...
    def predict(self):
        ...
        if len(self.memo_bank) < enable_thresh:
            ...
            if mig_vel is not None:
                ...
                # freeze-1l accel path: if the destination estimated a constant
                # acceleration for this migrated track (|a| above the noise
                # floor; None/0 otherwise), coast with constant-acceleration
                # kinematics along the motion direction: x = x0 + v0*t +
                # 0.5*a*t^2. a == 0 reduces this EXACTLY to the CV coast (else),
                # so constant-speed migrations are byte-identical.
                _a = getattr(self, '_migrated_accel_mps2', None)
                if _a:
                    t = spf * steps
                    v0 = float((mig_vel[0] ** 2 + mig_vel[1] ** 2) ** 0.5)
                    if _a < 0.0 and v0 > 0.0:
                        # Do not reverse past a stop when decelerating.
                        t = min(t, v0 / (-_a))
                    disp = v0 * t + 0.5 * _a * t * t
                    if v0 > 1e-6:
                        ux, uy = mig_vel[0] / v0, mig_vel[1] / v0
                    else:
                        ux, uy = 0.0, 0.0
                    pred[0] = self.memo_bank[-1][0] + ux * disp
                    pred[1] = self.memo_bank[-1][1] + uy * disp
                else:
