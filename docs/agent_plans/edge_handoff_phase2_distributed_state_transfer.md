...
- [x] **Third regression found and fixed (`653082ed`):** rerun after both fixes above now completes the overtake (no more permanent stuck state) but clips the ambulance once mid-maneuver. Instrumented `overtake_management`'s dry-run rejection (`ebd5e18d`, temporary) — confirmed `trigger_carla_id=122` (the NPC) every time, not the ambulance. Quantified: `collision_time_ahead=4` was shared between the ambulance-braking trigger (ego's own path, correctly tuned for a stationary hazard) and the candidate-lane-change safety check inside `overtake_management` (checked against the NPC, 18 m/s constant). Same 4s TTC threshold demands ~72m clearance from the fast NPC vs. a much shorter braking distance from the stationary ambulance — ego committed to the overtake with the NPC exactly 70.7m away (≈18×4), by which point only ~10m of the ambulance's 44m braking buffer remained. Fixed by giving the adjacent-lane check its own threshold, `overtake_lane_safety_time_ahead` (default 2.0s), selected in `collision_manager()` via the existing `adjacent_check` flag — no call-site changes needed. **All Step 1 measurements taken before this fix are invalid — re-run required. Remove the temporary dry-run instrumentation once this run confirms the fix.**
