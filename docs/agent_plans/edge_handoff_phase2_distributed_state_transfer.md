...
- [ ] **Confirmed broken, not just unchecked (2026-08-22 retroactive audit of `nAHj5t.log`, the successful two-edge `-eo` run):** `edge_profiler_<ts>.json` is not written. Zero profiler files exist anywhere in the repo dated today; every file on disk predates this session by months. The log has no `end_scenario`/`Edge_EndScenario` line at all, despite the scenario's `finally` block calling `fc.end_scenario()` for every fusion client. Needs its own investigation — not diagnosed further yet.
...
- [~] Two-edge `-eo` bring-up on the right-merge scenario, handoff block disabled — **infrastructure confirmed working, but not run as specified.** First attempt (2026-08-22) hit a false failure: `start_actors.sh`'s `-eo` readiness gate polls the base log for the literal string `[EDGE-ONLY]`, which only existed in `openscenario_3_edge_worldfusion.py`/`openscenario_3_edge_late_fusion.py` — nobody added it to the right-merge scenario file because two-edge `-eo` had never been run on it (exactly the F7 gap). Log showed both edges registered, fusion clients connected, actors spawned, RSUs/locales built — the simulation was healthy; the shell script's 60s timeout killed a working run. Fixed: added the matching print (`0767ed3a`). Retry succeeded (`nAHj5t.log`, full 700-tick completion) — **but this ran with the handoff block active, not disabled as this item specifies** (no mechanism to disable it exists). Running with it active confirmed F2's predicted silent degradation exactly: `transfer_obstacle_state: no KF for carla_id=122` repeats for the entire run, ending in `[SCENB] no obstacle handoff fired`. So: two-edge `-eo` infrastructure bring-up works; the handoff-disabled variant this item calls for was never actually run.
...
- [x] **Sequential regression found and fixed (`fef25d41`):** first Step-1 sequential rerun ended in an ego/ambulance collision. Traced to a `46216a12` merge regression in `PredictionLateFusionEdge._advance_actors()` — the RSU perception refresh loop (`rsu.update_info(); rsu.run_step()`) that existed inline in `run_step()` at run-12 (`6cefda1a`) was dropped when that block was extracted into `_advance_actors()`. Confirmed via `[SCENB-DBG]` logs: edge 0's tracker had `n_trk=0` from tick 165 through tick 690 across three consecutive runs, and the obstacle handoff never fired in any of them ("no obstacle handoff fired" every time). Checked `WorldFusionEdge`/`_PluggableEdgeBase` — both correctly gate an RSU refresh in their sequential paths; the gap was isolated to the late-fusion class. Fixed by restoring the loop. **All Step 1 measurements taken before this fix are invalid — re-run required.**
- [x] **Second regression found and fixed (`b954dbc9`):** rerun after the RSU fix succeeded at the mechanism level — obstacle handoff fired at tick 161, `npc_x=98.3`, exactly matching run-12 — but ended in a *different*, sustained ego/ambulance creep-collision (13s, hundreds of `Collision` warnings). Traced to the already-flagged curved-road suppression bug (`behavior_agent.py:1573`, previously Tyler-scope/deferred): `curved=True` latched continuously from tick 440 through the collision window (vs flickering ~15-20% of samples with no sustained run in the two immediately-preceding non-colliding runs), permanently forcing car-following regardless of `overtake_allowed`/`overtake_counter`. Fixed by removing `potential_curved_road` from the step-8 guard, per jrapp's decision to apply the known fix now rather than wait for Tyler sync. **All Step 1 measurements taken before this fix are invalid — re-run required.**
- [x] **Third regression found and fixed (`653082ed`), confirmed and instrumentation removed (`2635fa03`):** rerun after both fixes above now completes the overtake (no more permanent stuck state) but clips the ambulance once mid-maneuver. Instrumented `overtake_management`'s dry-run rejection (`ebd5e18d`/`325e9b31`, temporary) — confirmed `trigger_carla_id=122` (the NPC) every time, not the ambulance. Quantified: `collision_time_ahead=4` was shared between the ambulance-braking trigger (ego's own path, correctly tuned for a stationary hazard) and the candidate-lane-change safety check inside `overtake_management` (checked against the NPC, 18 m/s constant). Same 4s TTC threshold demands ~72m clearance from the fast NPC vs. a much shorter braking distance from the stationary ambulance — ego committed to the overtake with the NPC exactly 70.7m away (≈18×4), by which point only ~10m of the ambulance's 44m braking buffer remained. Fixed by giving the adjacent-lane check its own threshold, `overtake_lane_safety_time_ahead` (default 2.0s), selected in `collision_manager()` via the existing `adjacent_check` flag — no call-site changes needed. **Confirmed across 3 consecutive sequential runs** (`XuwE8k`, `exyzDi`, `vG76bj`): 0 collisions, `ghost_brake_events=0`, `MEASURED=29 ticks` identical in all three. One run reproduced the `carla_id=-1` variant of the NPC rejection (intermittent identity-resolution miss on the unbeaconed NPC); position data confirmed it was the same NPC track each time — the ambulance's position never moved and was never the trigger in any run. Temporary diagnostic logging removed in `2635fa03`. **Step 1 cold-baseline measurement is now valid: `MEASURED=29 ticks` with `handoff_warm_import: false`.**
...
- [x] Import-side reconciliation: on the first detection that associates to an injected track, merge rather than spawn a duplicate. **Original analysis was wrong** (estimated the warm track would be pruned ~tick 185, safely before real detection ~tick 243 — based on the 60m geometric proxy, not the actual first-detection tick). **Confirmed finding, see "Step 1 conclusion" below: reconciliation fails structurally under the current primitive KF-injection mechanism, via two independent, now-fixed bugs plus one deeper, unfixed limitation.**
- [x] Add the serialize → deserialize round-trip to both daemon methods — `983644ae` (confirmed: `payload_bytes()=1050`, `len(pickle)=1696`)
- [x] Instrument `first_track_publish_tick[carla_id]` on the edge manager — `983644ae`, **fixed in `d6326dd3`**: cid-keyed matching was broken (unbeaconed NPC always has `cid=-1`, shared by every unidentified object — both the cold and warm run logged `carla_id=-1` at nearly the same tick, a false near-zero delta). Reworked to position-gated matching: scenario calls `edge.note_target_position(npc_carla_id, npc_xy)` every tick; `_ab3d_history_to_trajs` matches live tracks within 5m and records the real carla_id. **Both prior runs (167/168) are invalid — re-run required.**
...
- [x] Negative control harness: run warm-import on vs off — 1 cold + 1 warm run, both with `[TRACK_BIRTH]`/`[TRACK_DEATH]`/`[TRACK_UNMATCHED]`/`[TRACK_DRIFT]` instrumentation (`60136424`/`c3a75764`/`7997176d`, temporary). Sufficient to characterize the mechanism; **≥3-run statistical rigor deferred — see "Step 1 conclusion."**
- [x] **Gate: resolved, with a reframed answer.** `MEASURED` delta was 0 (190 ticks, cold and warm identical) — but not because H1's physical claim is false. It's because `_ab3d_history_to_trajs` (the only place a track is added to `tracked_trajectories`/predictions) only runs on ticks where a *new* detection drains, so the injected track is invisible to predictions regardless of injection quality until the same external event that would produce a cold-start track anyway. Tracing *why* surfaced two real, now-fixed bugs and one deeper, unfixed limitation — see "Step 1 conclusion."
- [x] Check for duplicate-ghost regressions: `ghost_brake_events` — 0 in every run, all conditions (cold, warm, pre-fix, post-fix). No duplicate-ghost risk observed at the current operating point.
- [x] Commit immediately on a validated run — all fixes and diagnostics committed incrementally throughout (see commit list in "Step 1 conclusion").

#### Step 1 conclusion (2026-08-22)

**What "delta = 0" actually meant.** The original measurement question — "does edge1 publish a track for the NPC sooner with warm import on?" — turned out to be the wrong question. jrapp's framing corrected this: the thesis isn't about *when* a track first appears, it's about whether the track that appears *at* first detection already carries a converged trajectory (high hit-count, correct velocity) instead of starting from scratch. Investigating that reframed question surfaced three findings, of increasing depth:

1. **Bug (fixed, `22f455a0`): off-by-one in the zombie filter.** `_ab3d_history_to_trajs`'s `live_tids = {trk.id + 1 for trk in self.tracker.trackers}` never matched `output()`'s actual row format (`[trk.id]`, no offset, unchanged across the file's full git history). Every genuinely-alive track was treated as a zombie and dropped unless some *other* track's `id - 1` happened to also be alive — coincidentally masking the bug in normal multi-object scenes. Also present in `stale_track_suppression`'s `_stale_tids` and the `ZOMBIE_TRACE` diagnostic (3 sites total, same file).
2. **Bug (fixed, `22f455a0`): `max_age=6` (tracker calls, not world ticks) is far too short a survival budget for a warm-imported track waiting on a real detection that can arrive 25-70+ ticks later, especially since the call-count clock advances on *any* object's detection on that edge, not specifically the handed-off object's.** Fixed by giving warm-imported tracks a configurable negative starting `time_since_update` (`handoff_track_grace_ticks`, default 60) — extends survival without loosening ghost suppression for normal cold-started tracks. Confirmed via `[TRACK_DEATH]` instrumentation: with the fix, a track that died at frame_count 166 (pre-fix) survived to frame_count 226 (post-fix) — exactly the intended +60.
3. **Limitation (not fixed, structural): even with both bugs fixed and full survival, reconciliation still isn't guaranteed.** `[TRACK_DRIFT]` instrumentation showed the injected KF's coasted position carries a persistent ~6-15m bias relative to the NPC's true ground-truth position, present from the very first tick after injection and not closing over the track's lifetime. AB3DMOT's actual association gate (`thres=2.0`, ≈2m) is far tighter than that bias, so the real detection reliably fails to associate with the injected track and spawns a new one instead — independent of survival time. Simple constant-velocity coasting from a several-tick-stale export snapshot isn't accurate enough to satisfy a tight kinematic gate over a long, unobserved window.

**Why we stopped here rather than fixing #3.** Tyler Landle's dissertation proposal (`Dissertation_Proposal (1).pdf`, submitted for defense the week of 2026-08-24) describes Khonsu's actual migration protocol (Section 6.3.3): an explicit prepare-commit transfer with a monotonically increasing per-track epoch, where the destination's imported state becomes authoritative at an explicit commit event (the vehicle crossing the locale boundary) — not by waiting for the tracker's own association gate to accept a coasted KF as a match for a new detection. That design sidesteps finding #3 entirely: it doesn't depend on organic re-association at all. What we were testing (direct KF injection into `self.tracker.trackers`, hoping AB3DMOT's own detection pipeline picks it up) is a more primitive mechanism than Khonsu's specified protocol, and finding #3 is best understood as "the primitive mechanism doesn't have Khonsu's ownership/epoch layer yet" rather than "the physical claim in H1 is false." Also relevant: Khonsu's trigger fires on imminent locale-boundary crossing (short lead, immediate continuous re-observation after arrival), whereas Scenario B's `transfer_obstacle_state` shares an *unmanaged, unbeaconed* obstacle well before the destination can see it locally at all — a longer, unobserved coast with no continuous re-anchoring, which is a harder version of the problem than Khonsu's current scope covers.

**Regression-scope check: do the two fixed bugs touch any of Tyler's reported dissertation results?** Bounded check, not a full audit:
- Both bugs were introduced by Tyler's own commit `1f970053` (2026-07-08, "Multi-edge predictive latent migration: live handoff, eval plan, scenarios"), which directly modified `edge_manager_prediction_late_fusion_ab3dmot_linear_predictor.py` (462 lines).
- **Table 6.1** (the full/one-frame/cold displacement-error microbenchmark) is generated by `migration/harness.py` (also added in `1f970053`) — confirmed by inspection to be a fully synthetic, standalone harness (`traj_straight`/`traj_turn`/`traj_brake`/`traj_lane_change` feeding a tracker object directly) with no reference to `run_step`, `_advance_actors`, or `_ab3d_history_to_trajs`. **Unaffected** — never touches the buggy code path.
- **The two-locale MTR/SSM live handoff** (zero collisions, ~440 B memo, "highway right merge and blind overtake") uses the Mamba/SSM tracker backend (`_PluggableEdgeBase`, via `openscenario_3_multi_edge_mamba.py`, also added in `1f970053`) — confirmed by direct file inspection to be a separate file with neither bug (no `trk.id + 1` pattern; `_advance_vehicles` already correctly gates the RSU refresh loop). **Unaffected.**
- **B0.1** ("live edge migrates KF state... round-trip verified", the classical-backend vehicle-ownership handoff Section 6.4 cites) — `current_state.md`'s own record dates this DONE on 2026-07-05, three days *before* `1f970053` (2026-07-08) introduced the bugs into this file. **Unaffected by timing** — the validated result predates the bugs' existence.
- **Caveat:** this rules out the *original* validation runs for these three results. It cannot rule out a *later* re-run (e.g., a final regression pass before submission) against current `develop`, which carried both bugs until today's fixes — the off-by-one bug in particular is easy to miss since it's often masked by coincidental ID-adjacency in scenes with multiple simultaneous tracks. Worth a two-minute question to Tyler if it comes up, not worth interrupting defense prep to chase further ourselves.

**Commits this investigation produced:** `d6326dd3` (fix broken cid-keyed instrumentation), `ebd5e18d`/`325e9b31` (temp overtake dry-run diagnostics, later removed in `2635fa03`), `653082ed` (separate overtake TTC threshold — unrelated collision fix, see Step 0), `22f455a0` (off-by-one + max_age grace period fixes, the actual reconciliation-mechanism fixes), `60136424`/`c3a75764` (temp AB3DMOT birth/death/unmatched instrumentation), `7997176d` (temp position-drift trace). Temporary diagnostics in `AB3DMOT_libs/model.py` (`60136424`/`c3a75764`) and the drift trace in the late-fusion file (`7997176d`) are still present — remove once this finding is fully written up and no further investigation is planned.
