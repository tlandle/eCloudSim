...
## Degenerate runs excluded automatically (not counted as failures)
A CARLA startup transient (the map fails to load, so ScenarioManager has no world) writes a RUNROW but never spawns actors or steps the ego. Such a run is INVALID, not a not-completed failure. The lander (scripts/khonsu_design_extract.py) now excludes any log whose text shows the "is not found in your CARLA repo" / "has no attribute 'world'" startup error, or has zero GT-inject actors, or has no EGO-DBG lines, printing "INVALID degenerate run EXCLUDED" to stderr. The campaign launcher retries such a cell once before moving on. First instance: freeze-1k Block A hl_handoversnap_r7 (map-load transient; quarantined as .INVALID_town01 and re-run). This is the only Block A cell affected; with it excluded/re-run, handoversnap is 9/9 valid completions pending the re-run for a full 10.
