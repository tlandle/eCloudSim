...
- CETUS CHAIN (all whole-block-per-host, three live tmux): capacity (running) -> flowarms (waits frozen_1n_capacity/_done) -> t12 (waits frozen_1n_flow_arms/_done). ATLAS: regions freeze-1n grid running in idfix_wt (frozen_1n_t25, first row t25_cold_v6_s1 valid) + landloop (frozen1n_t25_rows.csv every 10 min). Flow-arms first row to be confirmed when capacity frees.
