...
ROUND-TRIP RELAY RE-RUN (peer: paper's link params are one-way fibre latency spent as a round trip): added a second netem filter on sport 50771 (response direction) to atlas_relay_smoke.sh netem_apply, keeping the port filter so CARLA is untouched; commit_ms should ~double to ~2x one-way. Re-running (bsbfn44v2). LAUNCH LESSON: putting sudo/pkill in the LAUNCHING Bash command is sandbox-denied (exit 1, empty output); launch the smoke with a bare `bash script > file` (cleanup/sudo/tc live INSIDE the script, which runs fine backgrounded).
...
PRE-CAMPAIGN GATE CLEAR (all 3 ordered items done). The freeze-1m whole-campaign rerun is unblocked from the code side (Defect A/B, shift mapping, transport). AWAITING peer/Tyler go to launch (large multi-block run, both machines: headline, trigger, record ablation, loaded, T25, maneuvering sweep {0,-5,-11,-17,-22,-26,-30}x10 seeds, T25b, non-central). BEFORE the blocks that need them: add identity-settling HANDOFFROW columns (identity_at_commit_matches / ticks_to_settle / settled) + identity-key the FDE/record-ablation/headline landers (key on tracklet identity, record resolved carla_id).
