...
- CETUS LANDINGS on 1j (2ac43280): loaded headline N=20 (30 rows) ALL ARMS 0/5 including warm and reactive = the NS3_LUT_N override defect (forced N realizes >= 800 ms), block INVALID, not the deployed-load measurement; item 1 (override fix) moved to right after the 1k tag, before any loaded rerun (the deployed-load headline and the freshness sweep depend on it; the paper's reactive 4/10 at deployed load is a projection with no valid measurement path yet). Table 8 on 1j: burst_warm 2/5 (1g 5/5), density warm 5/4/2 of 5 at n=2/4/8 (1g 5/3/5), edgewarp/cold 0: multi-crossing warm cells REGRESSED 1g->1j while single-crossing warm held; suspicion: 1h identity merge under several simultaneous imports (8 m position gate on bunched platoon vehicles: mis-association/duplicates) or the publish gate with several shadows; diagnosis ordered (contact actor, duplicates per carla_id, association events, PUBGATE, dbl_ticks vs a clean 1g burst run); Table 8 held out of figures. Cetus tail (visible matrix -> netem -> bisect) continues on 1j; loaded-headline and Table 8 reruns on 1k after item 1 and the burst diagnosis.
