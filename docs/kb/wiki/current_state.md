...
SHIFT MAPPING SIGNED OFF (peer). Consequence: the boundary moved to ~T_avail 0, so the OLD maneuvering shift set {10,0,-10,-18,-26} is now 4/5 in the delivered region (flat panel). NEW SHIFT SET PROPOSED (peer sign-off pending) from crossing = 85.9 - 1.69*shift, dec_arr ~125 (shift-independent), T_avail = 1.95 + 0.085*shift: shifts {-5,-11,-17,-22,-26,-30} -> predicted T_avail {1.53,1.02,0.52,0.10,-0.24,-0.58} (3 transition, 1 boundary, 2 blind). LANDER RULE (confirmed by design): the maneuvering lander (not yet built) computes T_avail PER-RUN = (dec_arr - crossing)*0.05 from that run's own EGO-DBG (first tick ego x<=307) + HANDOFFROW crossing_tick, never a nominal per-shift value; shift only selects the operating point, dec_arr noise scatters points along the real axis. HANDOFFROW already has crossing+first_use; only the identity-settling columns are new.

ORDERED pre-campaign work (peer): (1) Defect A - DONE (cadence, no code change; trigger 3-cycle reservation KEPT as design margin, paper adds measured ~2-cycle commit-to-first-use); (2) shift mapping - DONE (fixed re-derivation signed off; new maneuvering shift set proposed); (3) relay smoke - RUNNING (atlas_relay_smoke.sh: relay_server on 50771 up + port-probe readiness passed, relay.log shows PREPARE/COMMIT RPCs = relay carrying traffic in d=0 cell; port-filtered netem on dport 50771 so CARLA undelayed; cells 0/3/20/50 ms, TRANSPORT_MEASURING=1; asserts [XFERROW]>0, [XFER_FALLBACK]=0, prepare_ms ~2x delay; frozen_1m_relay/_report.txt). Then the whole-campaign rerun on freeze-1m. Defect B no longer blocks.
