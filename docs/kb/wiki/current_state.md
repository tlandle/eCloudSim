...
- 10-file overlay (scratchpad *_wired + behavior_agent_1k + edge_manager_merged): behavior_agent, EM, pluggable_base, runner, mtr, wrapper, tracker, tracklet, factories, payload. Full 19-cell targeted smoke re-running with corrected wrapper (bq0ct3rol). On gate PASS: sync 10 files to fix_wt, tag khonsu-eval-freeze-1k (message extended "per-frame source ticks carried in the record; velocity and coast projection use exact frame timing"), push, launch whole campaign.
