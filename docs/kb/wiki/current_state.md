...
Full 7-shift sweep DEFERRED: the maneuvering panel is now a SUCCESS-RATE run at a tight operating point (a=5 shift-40 x paired seeds), not a shift sweep; the shift sweep may still frame the constant-speed T_avail panel. HELD for the seed band + peer's nod on the operating point.
