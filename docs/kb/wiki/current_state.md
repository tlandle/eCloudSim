...
- OPEN DECISION (peer/Tyler): (a) higher accel {4,5,6} at shift -40 to find the lowest accel where kf collides / times out while warm completes = closed-loop panel; or (b) forecast-quality panel (warm meet_spd/FDE captures accel, kf does not), FDE not collision. Defaulting to (a) pending the peer; proposed a in {4,5,6} probe at shift -40 (~30 min), HELD for the peer's nod.
Full 7-shift sweep HELD pending: the accel/panel decision above + the SEED decision below.
