...
FIX DIRECTION (not built; the core migration mechanism, so awaiting Tyler/peer read on approach): the destination must produce a forecast from the migrated state DURING the blind window, independent of local detection - do NOT prune imported tids' trajectories while coasting, seed from memo_bank every tick regardless of local update, source the tracklet from lost_tracklets as well as tracked_tracklets, and run MTR on that coasted migrated state (the "prepared shadow that continues at the destination" of the paper). The current build ties the destination forecast to local re-detection, defeating migration in exactly the window it exists for.

DO NOT start the whole-campaign rerun until this is fixed and the attributed verify shows warm first-DST-MTR within ~1 cycle of commit while kf stays late. Defect A, the finer probe sign-off, and the relay smoke also wait.
