...
- On accel criterion OK: sync 10 files to fix_wt (at freeze-1j 9ca76adf; overlay set fully specifies the changed files), commit, tag khonsu-eval-freeze-1k, push; commit develop gate (khonsu_accept inv8 rewrite + accel + k_ prefix) + README; then 1k campaign (A/B/E/F/T25/A11-20/B11-20/faults Atlas; Table8-10seed/matrix/netem/bisect cetus; NOT band/load/loaded/freshness -> 1l). block A edgewarp = MIGRATION_MODE=edgewarp_full; factorial keeps edgewarp = edgewarp_snap. freeze-1l = override fix + ONCOMING_SHIFT_X + kf_final + repl_final (priority) + logging fields, own smoke.
