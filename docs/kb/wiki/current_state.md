...
CAMPAIGN STARTED: maneuvering first-run smoke RUNNING (atlas_maneuver_smoke.sh, warm vs kf @ shift=-17 a=3.0, checks warm!=kf under accel + warm CVMIGRATED=0 / kf>0 + [MIG_IDENT] live). Full 7-shift sweep HELD pending (a) smoke differentiation result, (b) accel-level pick (lowest commanded level that diverges), (c) the SEED decision below.
