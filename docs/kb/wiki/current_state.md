...
FIXED SHIFT PROBE RUNNING (Atlas, atlas_1m_probe.sh, fixed edge-manager overlay = coast+ghost fixes, same shift set {10,0,-10,-18,-26} x {warm,kf}, scenario openscenario_1_accel_gt const 12). Re-derives both quantities per shift: T_avail axis (decision_arrival - crossing)*0.05 (geometry, should match stale) and the blind/delivered boundary (first_use <= decision_arrival), now set by commit+2-cycle delivery not local re-detection. Peer signs off the mapping when this lands. Note: the accel scenario uses the LIVE accel runner (runner_wired overlays flow only), but the Defect B fixes are in the edge manager (overlaid for both), so the fixed destination behavior applies.

ORDERED pre-campaign work (peer): (1) Defect A - DONE (resolved as cadence; NO code change; trigger 3-cycle reservation KEPT as a design margin, paper adds measured ~2-cycle commit-to-first-use); (2) shift mapping - RE-DERIVING on fixed overlay (Atlas probe running); (3) relay smoke. Then the whole-campaign rerun on freeze-1m. Defect B no longer blocks.
