...
- PENDING: peer decision on parallelize (move ACC/T25 to cetus) + whether faults runs under 1k. cetus bringup (checkout bc26cd3e + protoc) for Table8@10/matrix/netem/bisect. freeze-1l build: T12 override fix -> repl_final (priority) -> kf_final -> ONCOMING_SHIFT_X -> logging fields, then a 1l smoke (1 cell/mode+knob + warm x1).
