...
- Cetus: reachable (tlandle@143.215.184.49), has the tag. BLOCKED: still running the stale cetus_1j_tail.sh (superseded 1j data, ~8.5GB GPU). Asked peer for OK to kill it, then cetus 1k = checkout tag + protoc + ACC1-10 -> T25 -> Table8@10 -> 5.3 matrix -> netem -> bisect.
