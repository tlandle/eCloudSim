...
def parse_config():
    ...
    parser.add_argument('--fix_random_seed', type=int, default=None, required=False, help='seed for np, random and torch')
    ...
    parser.add_argument('--freeze_mtr', action='store_true', default=False, help='')
...
def build_layered_optimizer(model, opt_cfg):
    if opt_cfg.OPTIMIZER == 'Adam':
        optimizer = torch.optim.Adam(
            [each[1] for each in model.named_parameters()],
            lr=opt_cfg.LR, weight_decay=opt_cfg.get('WEIGHT_DECAY', 0)
        ...
        optimizer = torch.optim.AdamW(
            [
                {'params': model.motion_transformer.parameters(), 'lr': opt_cfg.LR/100, 'weight_decay':opt_cfg.get('WEIGHT_DECAY', 0)},
                {'params': model.motion_aggregator.parameters(), 'lr': opt_cfg.LR, 'weight_decay':opt_cfg.get('WEIGHT_DECAY', 0)},
            ]
        )
    else:
        assert False
...
def main():
    ...
    output_dir = cfg.ROOT_DIR / 'MTR' / 'output' / cfg.TAG / args.extra_tag
    ...
    if args.fix_random_seed:
        logger.info('Using seed %d' % args.fix_random_seed)
    ...
    model = MotionTransformerWithMultiEgoAggregation(config=cfg.MODEL)
    ...
    if os.path.exists(cfg.DATA_CONFIG.PRETRAINED_MOTION_TRANSFORMER):
        print(f'Using pretrained Multiego MTR at {cfg.DATA_CONFIG.PRETRAINED_MOTION_TRANSFORMER}')
        model.load_params_from_file(cfg.DATA_CONFIG.PRETRAINED_MOTION_TRANSFORMER, logger)        
        optimizer = build_layered_optimizer(model, cfg.OPTIMIZATION)

        # Whether to freeze MTR.
        if args.freeze_mtr:
            logger.info("Freezing MTR weights")
            for param in model.motion_transformer.parameters():
    ...
    from test_multiego import repeat_eval_ckpt, eval_single_ckpt
    repeat_eval_ckpt(
