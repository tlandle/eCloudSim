...
class Traffic:
    def __init__(
        ...
        if x_initial is None:
            ...
            positions_init = 10 * np.random.choice(
                numpos, size=self.numcars, replace=False
            )

            for i in range(0, numcars):
                self.cars_on_road.append(
                    Car(
                        self.dt,
                        self.numlanes,
                        int(positions_init[i]),
                        np.random.randint(0, numlanes),
            ...
            if len(indices_of_interest) <= 1:
                ...
                for j in range(0, len(indices_of_interest)):
                    for k in range(j + 1, len(indices_of_interest)):
                        distance_between = (
                            indices_of_interest[j] - indices_of_interest[k]
                        )
                        ...
                        if (distance_between < 3) and (distance_between > 0):
                            self.cars_on_road[coord_j].pos_x = (
                                self.cars_on_road[coord_k].pos_x + 3
                            ) % self.map_length
                        elif (distance_between > -3) and (distance_between < 0):
                            self.cars_on_road[coord_k].pos_x = (
                                self.cars_on_road[coord_k].pos_x + 3
                            ) % self.map_length
                        self.update_grid_occupancies()
    ...
    def check_adjacent_occupancies(self, car):
        ...
        if car.lane < self.numlanes - 1:
            ...
            if (car.pos_x + margin_safety) > self.map_length - 1:
                ...
                grid_slice_upper = self.lane_vehicles[
                    0 : forward_ind_ahead + 1, car.lane + 1
                ]
                if np.sum(grid_slice_upper) > 0:
                    occupied_above = 1
            ...
            if car.pos_x < margin_safety:
                ...
                grid_slice_upper = self.lane_vehicles[
                    rear_ind : self.map_length - 1, car.lane + 1
                ]
                if np.sum(grid_slice_upper) > 0:
                    occupied_above = 1
        ...
        if car.lane > 0:
            ...
            if (car.pos_x + margin_safety) > self.map_length - 1:
                forward_ind_ahead = int((car.pos_x + margin_safety) % self.map_length)
                grid_slice_lower = self.lane_vehicles[
                    0 : forward_ind_ahead + 1, car.lane - 1
                ]
                if np.sum(grid_slice_lower) > 0:
                    occupied_below = 1
    ...
    def check_ahead(self, car, margin_ahead=15):
        ...
        grid_slice_required = self.lane_vehicles[
            int(car.pos_x + 1) : indice_ahead, int(car.lane)
        ]
        vehicle_ahead = None
    ...
    def check_collisions(self):
        ...
        for i in range(0, self.numlanes):
            vehicle_indices = self.lane_vehicles[
                np.where(self.lane_vehicles[:, i] != 0), i
            ]
            ...
            for j in range(0, len(vehicle_indices)):
                ...
                for k in range(j + 1, len(vehicle_indices)):
                    ...
                    if vehicle_j_pos <= collision_distance:
                        distance = (
                            vehicle_k_pos + collision_distance
                        ) % self.map_length - vehicle_j_pos
                    elif vehicle_j_pos >= self.map_length - collision_distance:
                        distance = (
                            vehicle_j_pos + collision_distance
                        ) % self.map_length - vehicle_k_pos
                    else:
                    ...
                    if abs(distance) <= collision_distance:
                        collisions_rec[
                            vehicle_indices[j] - 1, vehicle_indices[k] - 1
                        ] = 1
                        collisions_rec[
                            vehicle_indices[k] - 1, vehicle_indices[j] - 1
                        ] = 1
...
def main():
    ...
    while Traffic_Tracker.time < maxtime:
        ...
        # K-Means clustering
        kmeans = KMeans(n_clusters=numlanes, random_state=0).fit_predict(
            np.array(velocities_list).reshape(-1, 1)
        )
        ...
        plot_name = (
            "/home/chandramouli/Documents/collab-sandbox/comparison_three_test/clustering/"
            + "snapshot"
            + str(Traffic_Tracker.time / 0.2)
            + ".jpg"
        )
    ...
    scipy.io.savemat(
        "/home/chandramouli/Documents/collab-sandbox/comparison_three_test/clustering/States_x_clustering.mat",
        mdict={"X": xcars},
    )
    scipy.io.savemat(
        "/home/chandramouli/Documents/collab-sandbox/comparison_three_test/clustering/States_y_clustering.mat",
        mdict={"Y": ycars},
    )
    scipy.io.savemat(
        "/home/chandramouli/Documents/collab-sandbox/comparison_three_test/clustering/States_v_clustering.mat",
        mdict={"V": velocities},
    )
    scipy.io.savemat(
        "/home/chandramouli/Documents/collab-sandbox/comparison_three_test/clustering/States_tv_clustering.mat",
        mdict={"TV": target_velocities},
    )
    ...
    while Traffic_Tracker.time < maxtime:
        ...
        plot_name = (
            "/home/chandramouli/Documents/collab-sandbox/comparison_three_test/greedy/"
            + "snapshot"
            + str(Traffic_Tracker.time / 0.2)
            + ".jpg"
        )
    ...
    print("Final Lane: ", lanes_list)
    scipy.io.savemat(
        "/home/chandramouli/Documents/collab-sandbox/comparison_three_test/greedy/States_x_no_clustering.mat",
        mdict={"Xn": xcars},
    )
    scipy.io.savemat(
        "/home/chandramouli/Documents/collab-sandbox/comparison_three_test/greedy/States_y_no_clustering.mat",
        mdict={"Yn": ycars},
    )
    scipy.io.savemat(
        "/home/chandramouli/Documents/collab-sandbox/comparison_three_test/greedy/States_v_no_clustering.mat",
        mdict={"Vn": velocities},
    )
    scipy.io.savemat(
        "/home/chandramouli/Documents/collab-sandbox/comparison_three_test/greedy/States_tv_no_clustering.mat",
        mdict={"TVn": target_velocities},
    )
