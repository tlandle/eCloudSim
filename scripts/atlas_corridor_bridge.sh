...
R=$WT/evaluation_outputs/frozen_1n_corridor
...
land_and_pull () {
  ssh -o ConnectTimeout=15 "$CETUS" "mkdir -p $OUT; cd $WT && source ~/anaconda3/etc/profile.d/conda.sh && conda activate opencda310 && python scripts/khonsu_corridor_land.py $R --outdir $OUT --tau-ms 500 --complete-m 350 --tag khonsu-eval-freeze-1n --machine cetus" >>"$LOG" 2>&1 || true
  scp -o ConnectTimeout=15 "$CETUS:$OUT/frozen1l_corridor_rows.csv"      "$MOCK/frozen1n_corridor_rows.csv" >/dev/null 2>&1 || true
  scp -o ConnectTimeout=15 "$CETUS:$OUT/frozen1l_corridor_crossings.csv" "$MOCK/frozen1n_corridor_crossings.csv" >/dev/null 2>&1 || true
  local n=$(tail -n +2 "$MOCK/frozen1n_corridor_rows.csv" 2>/dev/null | wc -l)
