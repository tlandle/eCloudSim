...
run_one () {
  ...
  RUN_N=$((RUN_N + 1))
  echo "[$(date +%H:%M:%S)] $tag"
  env "$@" timeout -k 30 900 python ecav.py -t "${SCENARIO_OVERRIDE:-openscenario_1_flow_gt}" --apply_ml \
      > "$log" 2>&1 < /dev/null
  sleep 3
...
for rep in $(seq 1 "$REPS"); do
  run_one "v2_warm_r${rep}"      MIGRATION_MODE=warm
  run_one "v2_computed_r${rep}"  MIGRATION_MODE=warm TRIGGER_MODE=computed
  run_one "v2_reactive_r${rep}"  MIGRATION_MODE=reactive
  run_one "v2_kf_r${rep}"        MIGRATION_MODE=kf
  run_one "v2_edgewarp_r${rep}"  MIGRATION_MODE=edgewarp
  run_one "v2_cold_r${rep}"      MIGRATION_MODE=cold
done
# burst-admission arms on the platoon scenario
for rep in 1 2 3 4 5; do
  for m2 in warm edgewarp cold; do
    run_one "burst_${m2}_r${rep}" MIGRATION_MODE=$m2 SCENARIO_OVERRIDE=openscenario_1_burst_gt
  done
done
